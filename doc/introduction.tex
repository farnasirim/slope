...
are two important factors influencing the performance of the system. However,
optimizing each of these usually requires the system designer to make certain
compromises.
...
overhead coming with the extra functionality. For example MapReduce
\cite{dean2008mapreduce} can rarely benefit from the privilege of access to a
global shared memory in a synchronized manner.

Like MapReduce, An interesting property of these applications is that their
...
ones with data that is partitionable such that the
accesses from operations on one operation fall in that one partition only.
This is desirable because it eliminates the access locality problem, as
...
which take advantage of these conditions. By designing the algorithms
...
The platform will then ensure that each
