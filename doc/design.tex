...
\TODO{This is a simplified example, but for consistency, it would be good to show the object creation and the invocation of the migratable object's run() method (with the memory accesses, put and get, inside the run method...)}
...
\TODO{I don't think you previously discussed your assumptions on the machine capabilities - at least, not all in one place. You are certainly assuming they have RDMA-capable networking hardware, and it is reasonable to assume that such machines are also equipped with large physical memory. You are also assuming they are homogenous enough to all run the same application binary and access the objects without serializing/deserializing (so, same byte order). You also assume there may be some heterogeneity (i.e., some machines equipped with accelerators like GPUs). I think these are all reasonable assumptions, but you should pull them all together (in the introduction) and explain clearly the target environment for a system like Slope. }

\TODO{
    You need to provide more information on the implementation of Slope itself, not just the usage. Show the complete API for the migptr with a brief description of each of its methods. For the most important, createcontext method, give its pseudocode. Similarly, give the pseudocode for the custom allocate and deallocate functions. }
...
\TODO{
    I have some questions about the programming model at this point, which probably should be cleared up earlier (Chapter 1, along with system assumptions). Should the application spawn a new thread to execute the run() method of each migratable object? Does the run() method only return upon the destruction of the object (where migration to another machine results in local destruction)? Or can the run() method just perform some initialization/setup for the object and then return, leaving the object in a usable state where its methods can be invoked from other code. How does the requirement to invoke the object's run() method interact with the ability to compose migratable objects into larger migratable objects?
}
...
\TODO{This paragraph sounds like you first loop through all the pages in Preparation, and then loop through all the pages again to Send them. But after reading the rest of it, and based on the Figure, I think you loop through all the pages, and for each page, perform the Preparation and Send steps. }
...
\TODO{Re-order the presentation of the Prefill steps so that you start with Preparation, followed by Send, and finish with Dirty Page Detection. You will need to re-write parts of Send and Dirty Page Detection so that they make sense in that order. Right now, the details of dirty page detection depend on actions taken during Preparation and interact tightly with the Send steps, so it is harder to understand when it comes first.}
...
\TODO{relaxed no alloc/dealloc requirement during the migration, more thread safety requirements}
