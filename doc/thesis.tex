...
%% *** Put your Abstract here. ***
    ...
    \CHECK{in distributed software systems where managing the data or carrying
    out the computations on a single single machine becomes infeasible}.
    The ability to move in-memory resources across machines can directly impact the
    above metrics in applications, however altering the designs of
    data~structures to make them migratable is not only
    ...
    existing data~structures or any self-contained computation unit migratable.
    Our approach eliminates data
    serialization and can be applied to already existing data~structures. We
    use RDMA as the transport as it closely fits the requirements of Slope.
    We can incorporate Slope into already existing applications with
    separable and independent units of computations. We evaluate on blah \TODO{incomplete}.
