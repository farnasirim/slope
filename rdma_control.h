...
std::string
time_point_to_string(const std::chrono::high_resolution_clock::time_point &);

template <typename T> class RdmaControlPlane : public ControlPlane<T> {
...
public:
  ...
  virtual MigrationOperation::ptr init_migration(const std::string& dest,
    ...
    std::thread t([&, chunks] {
        ...
        std::cout << "before ping pong" << std::endl;
  ...
  RdmaControlPlane(const std::string &self_name,
      ...
      : self_name_(self_name), cluster_nodes_(cluster_nodes),
        keyvalue_service_(std::move(keyvalue_service)), dataplane_(nullptr),
...
std::vector<QpInfo> find_self_qps(const std::map<std::string, NodeInfo>& node_infos,
